...
<body>
	...
	<div class="Content">
		...
		<c:forEach items="${farmList}" var="farmlist" varStatus="status">
			...
			<c:set var="fundingPath" value="${cpath}/funding/fundingDetail?product_serial_num=${farmlist.product_serial_num}" />
			<c:set var="auctionPath" value="${cpath}/auction/auctionDetail?product_serial_num=${farmlist.product_serial_num}" />
			...
			<div class="content_detailall">
				...
				<div class="content_detail_secondAll">
					<%-- <div class="content_detail2_first" id="prodnameLink">${farmlist.product_name }</div> --%>
	...
	<script>
		...
		function searchDate() {
			$.ajax({
				url : "${cpath}/myPageUser/searchDate",
				data : {
